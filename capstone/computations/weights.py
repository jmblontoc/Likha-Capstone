...
def report_table_per_year(year):
    ...
    for b in barangays:
        ...
        query = OPTValues.objects.filter(opt__date__year=year, opt__barangay=b)
        sub_data = [b.name]
    ...
    for b in barangays:
        ...
        w_sw = query.filter(Q(nutritional_status=wfh_wasted) | Q(nutritional_status=wfh_sw)).aggregate(sum=Sum('values'))['sum']
        sub_data.append(w_sw)
        sub_data.append(round(w_sw / barangay_total, 2))
...
def report_table_sort_year():
    total = OperationTimbang.objects.dates("date", "year")
    total2 = []
